...
/* ============================================================
   VELNOC GEO ROI Simulator v7.1 — Design System Aligned
   ------------------------------------------------------------
   v7 ➞ v7.1 changes:
   + 디자인 시스템 v1.0 완전 정렬
   + 오크 그린 hex 교체: #0F4C3A → #2D5F4E (시스템 토큰)
   + Emerald 제거: #10B981 / #A7F3D0 → oak 계열로 흡수
   + 다크 배경(#0A0A0A) 박스 제거 → terracotta-soft / oak-soft
   + 그라데이션·shimmer 애니메이션 제거 (시스템 PART 1-5 금지)
   + 헤딩 폰트 Gowun Batang 적용 (vn-head 클래스)
   + Display Mode 결정타 1회: Hero h1 + 자산 격차 conclusion
   + 모든 컬러를 CSS 변수(var(--velnoc-*))로 호출
   ============================================================ */
...
export default function VelnocSimulatorV71() {
  ...
  return (
    <div className="min-h-screen w-full" style={{ background: 'var(--velnoc-stone)' }}>
      <style>{`
        ...
        .velnoc-sim .vn-head { font-family: var(--font-kr-head); font-weight: 400; letter-spacing: -0.015em; }
        .velnoc-sim .vn-display { font-family: var(--font-kr-display); font-weight: 700; letter-spacing: -0.02em; }
        ...
        .velnoc-sim input[type="range"]::-webkit-slider-thumb {
          ...
          background: var(--velnoc-ink-primary); cursor: pointer; border-radius: 50%;
          border: 3px solid var(--velnoc-stone); box-shadow: 0 0 0 1px var(--velnoc-ink-primary);
        }
        .velnoc-sim input[type="range"]::-moz-range-thumb {
          width: 18px; height: 18px; background: var(--velnoc-ink-primary);
          cursor: pointer; border-radius: 50%; border: 3px solid var(--velnoc-stone); box-shadow: 0 0 0 1px var(--velnoc-ink-primary);
        }
        ...
        .velnoc-sim .savings-glow { background: var(--velnoc-oak-deep); position: relative; overflow: hidden; }
        ...
        .velnoc-sim .asset-bar { 
          height: 4px; background: var(--velnoc-divider); overflow: hidden; position: relative;
        }
        .velnoc-sim .asset-bar-fill {
          position: absolute; top: 0; left: 0; height: 100%; background: var(--velnoc-oak);
      ...
      <div className="velnoc-sim grain">
        <div className="max-w-3xl mx-auto px-6 py-16 md:py-24">
          <header className="mb-20">
            <div className="text-xs tracking-[0.25em] uppercase mb-6" style={{ color: 'var(--velnoc-ink-tertiary)' }}>
              VELNOC · AI 검색 시대 ROI 시뮬레이터 v7
            </div>
            <h1 className="vn-display text-[2.5rem] md:text-[4rem] leading-[0.95] mb-6" style={{ color: 'var(--velnoc-ink-primary)' }}>
              5년 뒤,<br />
              당신의 마케팅비는<br />
              <span style={{ color: 'var(--velnoc-oak)' }}>어디에 남아있나요?</span>
            </h1>
            <p className="text-base md:text-lg max-w-xl leading-relaxed" style={{ color: 'var(--velnoc-ink-secondary)' }}>
            ...
            <div className="grid grid-cols-2 gap-3">
              {Object.entries(PRESETS).map(([key, preset]) => (
                <button key={key} onClick={() => setSituation(key)}
                  ...
                  style={{ border: situation === key ? '1px solid var(--velnoc-ink-primary)' : '1px solid #E7E5E0' }}>
                ...
                <div className="flex gap-2 pb-2" style={{ width: 'max-content' }}>
                  {Object.entries(INDUSTRIES).map(([key, ind]) => (
                    <button key={key} onClick={() => applyIndustry(key)}
                      ...
                      style={{ border: industry === key ? '1px solid var(--velnoc-ink-primary)' : '1px solid #E7E5E0' }}>
                ...
                {industry && industryAvg && (
                  <div className="mt-6 pt-6 border-t border-stone-200 fade-in">
                    ...
                    <div className="mt-3 p-3" style={{ background: benchmarkDelta > 0 ? '#FEF2F2' : '#F0FDF4' }}>
                      <span className="text-sm font-bold" style={{ color: benchmarkDelta > 0 ? '#B91C1C' : 'var(--velnoc-oak)' }}>
              ...
              <div className="flex gap-2">
                {[1, 3, 5, 10].map((y) => (
                  <button key={y} onClick={() => setPeriod(y)}
                    ...
                    style={{ border: period === y ? '1px solid var(--velnoc-ink-primary)' : '1px solid #E7E5E0' }}>
              ...
              <div className="grid grid-cols-3 gap-2">
                {INFLATION_OPTIONS.map((opt) => (
                  <button key={opt.value} onClick={() => setInflation(opt.value)}
                    ...
                    style={{ border: inflation === opt.value ? '1px solid var(--velnoc-ink-primary)' : '1px solid #E7E5E0' }}>
                    <div className="text-sm font-bold mb-1">{opt.label}</div>
                    <div className="tabular text-lg display font-black"
                      style={{ color: inflation === opt.value ? 'var(--velnoc-stone)' : opt.value === 0 ? '#78716C' : '#B91C1C' }}>
                      {opt.sub}
                    </div>
              ...
              <div className="bg-white" style={{ border: '1px solid #E7E5E0' }}>
                <div className="p-6 md:p-10">
                  ...
                  <div className="mb-6">
                    ...
                    <div className="live-tick display tabular font-black leading-none"
                      style={{ color: 'var(--velnoc-oak)', fontSize: 'clamp(1.75rem, 7vw, 3.5rem)' }}
                ...
                <div className="savings-glow flex flex-col md:flex-row md:items-center md:justify-between px-6 md:px-10 py-6 md:py-8 gap-2"
                  style={{ color: 'var(--velnoc-stone)' }}>
                  <div className="flex items-center gap-3">
                    <svg width="20" height="20" viewBox="0 0 16 16" fill="none">
                      <path d="M8 2 L8 12 M4 8 L8 12 L12 8" stroke="var(--velnoc-oak-soft)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
                    </svg>
                    <span className="text-xs md:text-sm uppercase tracking-[0.2em] text-stone-100 font-semibold">
                      벨녹 도입 시 절약
                    </span>
                  </div>
                  ...
                  <div className="h-80">
                    <ResponsiveContainer width="100%" height="100%">
                      <LineChart data={animatedChartData} margin={{ top: 10, right: 20, left: 0, bottom: 10 }}>
                        ...
                        <Tooltip
                          contentStyle={{ background: 'var(--velnoc-ink-primary)', border: 'none', borderRadius: 0, color: 'var(--velnoc-stone)', fontSize: 12 }}
                  ...
                  <div className="mt-6 pt-6 border-t border-stone-200">
                    <div className="flex items-center gap-4">
                      <button onClick={() => setIsPlaying(!isPlaying)}
                        ...
                        style={{ border: '1px solid var(--velnoc-ink-primary)', background: isPlaying ? 'var(--velnoc-stone)' : 'var(--velnoc-ink-primary)', color: isPlaying ? 'var(--velnoc-ink-primary)' : 'var(--velnoc-stone)' }}>
                        {isPlaying ? (
                      ...
                      <div className="flex-1">
                        ...
                        <div className="relative w-full h-1 bg-stone-200 overflow-hidden">
                          <div className="absolute top-0 left-0 h-full" style={{ width: `${drawProgress * 100}%`, background: 'var(--velnoc-ink-primary)' }} />
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="mt-6 flex flex-wrap gap-x-6 gap-y-2 text-xs">
                      ...
                      <LegendDot color="var(--velnoc-oak)" label="벨녹 구독" />
                    </div>
                    ...
                    <button onClick={() => setShowDefinitions(!showDefinitions)}
                      ...
                      style={{ border: '1px solid #E7E5E0', background: showDefinitions ? 'var(--velnoc-stone)' : '#FAFAF7' }}>
                      ...
                      <div className="fade-in mt-3 p-5 text-xs leading-relaxed space-y-4" style={{ background: '#FAFAF7', border: '1px solid #E7E5E0' }}>
                        <div className="pb-3 border-b border-stone-200">
                          ...
                          <ul className="space-y-1 text-stone-700 list-none pl-0">
                            ...
                            <li>· <span style={{ color: 'var(--velnoc-oak)' }} className="font-bold">{period}년 후 벨녹 구독료: 월 {formatWonNatural(velnocPriceAtEnd)}</span> (+{velnocPricePct}%)</li>
                          </ul>
                        </div>
                        ...
                        <DefRow color="var(--velnoc-oak)" title="벨녹 구독">
            ...
            <section className="mb-20 fade-in">
              ...
              <div className="bg-white" style={{ border: '1px solid #E7E5E0' }}>
                ...
                <div className="p-6 md:p-10 border-b border-stone-200">
                  <div className="text-xs uppercase tracking-[0.25em] mb-4" style={{ color: 'var(--velnoc-ink-tertiary)' }}>진짜 격차는 비용이 아닙니다</div>
                  <p className="vn-head text-xl md:text-3xl leading-tight" style={{ color: 'var(--velnoc-ink-primary)', fontWeight: 700 }}>
                    비용 차이는 시작일 뿐.<br />
                    <span style={{ color: 'var(--velnoc-oak)' }}>5년 동안 무엇이 쌓이느냐</span>가 진짜 격차입니다.
                  </p>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2">
                  ...
                  <div className="p-6 md:p-10" style={{ background: '#FAFAF7' }}>
                    <div className="text-xs uppercase tracking-[0.25em] mb-4" style={{ color: 'var(--velnoc-oak)' }}>
                    ...
                    <div className="space-y-5 mb-8">
                      ...
                      <div className="pt-2">
                        ...
                        <div className="text-xs" style={{ color: 'var(--velnoc-oak)' }}>자동 누적 · 내 소유</div>
                      </div>
                    </div>

                    <div className="pt-5 border-t border-stone-200">
                      <div className="text-sm font-bold" style={{ color: 'var(--velnoc-oak)' }}>
                        ✓ 멈춰도 자산 그대로
                      </div>
                ...
                <div className="p-4 md:p-8 pt-8 md:pt-10 border-t border-stone-200">
                      ...
                      <AreaChart data={animatedAssetData} margin={{ top: 10, right: 20, left: 0, bottom: 10 }}>
                        <defs>
                          <linearGradient id="velnocAssetGrad" x1="0" y1="0" x2="0" y2="1">
                            <stop offset="0%" stopColor="var(--velnoc-oak)" stopOpacity={0.25} />
                            <stop offset="100%" stopColor="var(--velnoc-oak)" stopOpacity={0} />
                          </linearGradient>
                        </defs>
                        <CartesianGrid stroke="#E7E5E0" vertical={false} />
                        <XAxis dataKey="year" type="number" domain={[0, period]}
                        ...
                        <Area type="monotone" dataKey="velnocAsset" stroke="var(--velnoc-oak)" strokeWidth={2.5} fill="url(#velnocAssetGrad)" isAnimationActive={false} connectNulls={false} />
                  ...
                  <div className="mt-4 flex flex-wrap gap-x-6 gap-y-2 text-xs">
                    ...
                    <LegendDot color="var(--velnoc-oak)" label="벨녹 (지수 성장)" />
                  </div>
                ...
                {/* Conclusion band */}
                <div className="p-6 md:p-10" style={{ background: 'var(--velnoc-oak-soft)', color: 'var(--velnoc-ink-primary)', borderTop: '0.5px solid var(--velnoc-divider)' }}>
                  <div className="text-xs uppercase tracking-[0.25em] mb-4" style={{ color: 'var(--velnoc-oak-deep)' }}>결론</div>
                  <p className="vn-display text-xl md:text-3xl leading-tight">
                    비용 차이는 {formatWonNatural(totalSavings)}.<br />
                    자산 가치 차이는 <span style={{ color: 'var(--velnoc-oak-deep)' }}>측정 불가</span>.
                  </p>
                  <p className="mt-4 text-sm md:text-base leading-relaxed" style={{ color: 'var(--velnoc-ink-secondary)' }}>
          ...
          {/* 기회비용 SECTION — 결정타 카드 (terracotta, 페이지당 1곳 한정) */}
          {stepReady.results && monthlySpend > 0 && totalSavings > 1000000 && (
            <section className="mb-20 fade-in">
              <div className="p-8 md:p-12" style={{ background: 'var(--velnoc-terracotta-soft)', color: 'var(--velnoc-ink-primary)', borderRadius: 8 }}>
                <div className="text-xs uppercase tracking-[0.25em] mb-6" style={{ color: 'var(--velnoc-terracotta)' }}>기회비용</div>

                <p className="vn-display text-2xl md:text-4xl leading-tight">
                  지난 {period}년간 마케팅에<br />
                  <span className="tabular" style={{ color: 'var(--velnoc-ink-primary)' }}>{formatWonNatural(currentTotal)}</span>을 지출하셨다면,
                </p>

                {opportunityCosts.length > 0 && (
                  <div className="mt-10 pt-8" style={{ borderTop: '0.5px solid rgba(196, 99, 60, 0.2)' }}>
                    <p className="vn-display text-xl md:text-2xl leading-tight mb-6" style={{ color: 'var(--velnoc-terracotta)' }}>
                      이 돈이면 이런 것들이 가능했습니다
                    </p>
                    <div>
                      {opportunityCosts.map((opp, i) => (
                        <div key={i} className="opp-row flex items-baseline justify-between py-4 gap-4">
                          <span className="text-base md:text-lg" style={{ color: 'var(--velnoc-ink-secondary)' }}>{opp.label}</span>
                          <span className="tabular text-base md:text-xl font-bold text-right" style={{ color: 'var(--velnoc-ink-primary)' }}>
                            {opp.value}
                          </span>
                        </div>
                      ))}
                ...
                <div className="mt-10 pt-8 text-base md:text-lg leading-relaxed" style={{ borderTop: '0.5px solid rgba(196, 99, 60, 0.2)', color: 'var(--velnoc-ink-secondary)' }}>
                  광고는 멈추면 사라지지만,<br />
                  벨녹은 멈춰도 <span style={{ color: 'var(--velnoc-ink-primary)', fontWeight: 600 }}>{period}년치 자산이 그대로 남습니다.</span>
                </div>
              </div>
            </section>
          )}
            ...
            <section className="mb-16 fade-in">
              {!showLeadForm && !submitted && (
                <button onClick={() => setShowLeadForm(true)}
                  ...
                  style={{ border: '1px solid var(--velnoc-ink-primary)' }}>
                  <div className="text-left">
                    <div className="text-xs uppercase tracking-[0.25em] mb-2" style={{ color: 'var(--velnoc-ink-tertiary)' }}>다음 단계</div>
                    <div className="vn-head text-xl md:text-2xl" style={{ color: 'var(--velnoc-ink-primary)', fontWeight: 700 }}>상세 PDF 리포트 + 무료 진단 →</div>
                  </div>
              ...
              {showLeadForm && !submitted && (
                <form onSubmit={handleSubmit} className="p-6 md:p-10 fade-in" style={{ background: 'var(--velnoc-surface)', border: '0.5px solid var(--velnoc-border)', borderRadius: 8 }}>
                  <h3 className="vn-head text-2xl md:text-3xl mb-2" style={{ color: 'var(--velnoc-ink-primary)', fontWeight: 700 }}>무료 AI 검색 가시성 진단</h3>
                  <p className="text-sm mb-8 leading-relaxed" style={{ color: 'var(--velnoc-ink-secondary)' }}>위 시뮬레이션의 상세 PDF 리포트와 함께,<br />귀사의 AI 검색 노출 현황을 진단해 드립니다.</p>
                  ...
                  <button type="submit" className="w-full py-5 text-lg font-medium tracking-tight hover:opacity-90 transition-opacity" style={{ background: 'var(--velnoc-oak)', color: 'var(--velnoc-surface)', borderRadius: 6, border: 'none' }}>
                    무료 진단 신청 →
                  </button>
                </form>
              )}
              ...
              {submitted && (
                <div className="p-10 md:p-12 text-center fade-in" style={{ background: 'var(--velnoc-oak)', color: 'var(--velnoc-surface)', borderRadius: 8 }}>
                  <div className="text-xs uppercase tracking-[0.25em] mb-6" style={{ color: 'var(--velnoc-oak-soft)' }}>접수 완료</div>
                  <h3 className="vn-head text-3xl md:text-4xl mb-4" style={{ fontWeight: 700 }}>잠시 후 메일을 확인해 주세요</h3>
                  <p className="text-base leading-relaxed" style={{ color: 'var(--velnoc-oak-soft)' }}>{form.name} 님,<br />24시간 안에 PDF 리포트와 함께 진단 결과를 보내드립니다.</p>
                </div>
              )}
            </section>
          )}
          ...
          <footer className="pt-12 border-t border-stone-300 text-xs text-stone-500 leading-relaxed">
            <div className="flex items-baseline justify-between flex-wrap gap-2">
              ...
              <div className="text-stone-400">v7.1 · Design System Aligned · 시뮬레이션 결과는 추정치입니다</div>
            </div>
          </footer>
        </div>
      </div>
  ...
  return (
    <div>
      <div className="flex items-baseline justify-between mb-2">
        ...
        <span className="live-tick tabular text-lg display font-black" style={{ color: 'var(--velnoc-oak)' }} key={`${label}-${revealKey}`}>
